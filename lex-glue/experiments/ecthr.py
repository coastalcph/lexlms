...
from convert_model import convert_bert_to_lf, convert_roberta_to_lf
...
class DataTrainingArguments:
    ...
    max_label_length: Optional[int] = field(
        default=16,
        metadata={
            "help": "The maximum total label sequence length after tokenization. Sequences longer "
                    "than this will be truncated, sequences shorter will be padded."
        },
    )
...
def main():
    ...
    if model_args.seq2seq:
        training_args.generation_max_length = data_args.max_label_length
    ...
    if training_args.do_eval:
        eval_dataset = load_dataset("lexlms/lex_glue_v2", data_args.task, split="validation",
                                    cache_dir=model_args.cache_dir)
        if model_args.seq2seq:
            eval_dataset = eval_dataset.rename_column('labels', 'label')
    if training_args.do_predict:
        predict_dataset = load_dataset("lexlms/lex_glue_v2", data_args.task, split="test",
                                       cache_dir=model_args.cache_dir)
        if model_args.seq2seq:
            predict_dataset = predict_dataset.rename_column('labels', 'label')

    # Labels
    label_list = list(range(train_dataset.features['labels' if not model_args.seq2seq else 'label'].feature.num_classes))
    labels_names = train_dataset.features['labels' if not model_args.seq2seq else 'label'].feature.names
    num_labels = len(label_list)


    if not os.path.exists(f'./{model_args.model_name_or_path}-lf'):
        if 'roberta' in model_args.model_name_or_path:
            convert_roberta_to_lf(model_args.model_name_or_path, f'./{model_args.model_name_or_path}-lf', max_text_length=2048)
        else:
            convert_bert_to_lf(model_args.model_name_or_path, f'./{model_args.model_name_or_path}-lf', max_text_length=2048)

    model_args.model_name_or_path = f'./{model_args.model_name_or_path}-lf'
    ...
    config.pooling_method = model_args.pooling_method
    ...
    def preprocess_function(examples):
        # Flatten case facts
        # Tokenize the texts
        texts = [' <sep> '.join(text) for text in examples['text']]
    ...
    def compute_metrics(p: EvalPrediction):
        ...
        from labeling_utils import fix_multi_label_scores
    ...
    if training_args.do_predict:
        ...
        if trainer.is_world_process_zero():
            ...
            cls_report = classification_report(y_true=y_true, y_pred=y_pred,
                                               target_names=labels_names + ['No Violation'],
                                               zero_division=0)
