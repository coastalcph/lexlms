...
def main():
    ...
    if training_args.do_train:
        train_dataset = load_dataset("lexlms/lex_glue_v2", "eurlex", split="train",
                                     cache_dir=model_args.cache_dir, use_auth_token=AUTH_KEY)
        if model_args.seq2seq:
            train_dataset = train_dataset.rename_column('labels', 'label')

    if training_args.do_eval:
        eval_dataset = load_dataset("lexlms/lex_glue_v2", "eurlex", split="validation",
                                    cache_dir=model_args.cache_dir, use_auth_token=AUTH_KEY)
        if model_args.seq2seq:
    ...
    # labels_names = [eurovoc_descriptors[label_code]['en'] for label_code in labels_codes]
    ...
    data_collator = DataCollatorForSeq2Seq(tokenizer=tokenizer, model=model,
                                           pad_to_multiple_of=data_args.max_label_length) \
