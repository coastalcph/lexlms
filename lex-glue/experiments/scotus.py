...
from convert_model import convert_roberta_to_lf, convert_bert_to_lf
...
def main():
    ...
    if training_args.do_train:
        train_dataset = load_dataset("lexlms/lex_glue_v2", 'scotus', split="train",
                                     cache_dir=model_args.cache_dir)

    if training_args.do_eval:
        eval_dataset = load_dataset("lexlms/lex_glue_v2", 'scotus', split="validation",
                                    cache_dir=model_args.cache_dir)
    if training_args.do_predict:
        predict_dataset = load_dataset("lexlms/lex_glue_v2", 'scotus', split="test",
                                       cache_dir=model_args.cache_dir)
    # Labels
    label_list = list(range(train_dataset.features['label'].num_classes))
    # labels_names = train_dataset.features['label'].names
    labels_names = ['Criminal Procedure', 'Civil Rights', 'First Amendment', 'Due Process', 'Privacy', 'Attorneys',
    ...
    def preprocess_function(examples):
        ...
        if model_args.seq2seq:
            label_batch = tokenizer(
                [labels_names[label].lower() for label in examples["label"]],
                padding=False,
            ...
            batch["labels"] = [label_list.index(labels) for labels in examples["label"]]
