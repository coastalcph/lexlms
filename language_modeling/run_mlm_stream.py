...
class DataTrainingArguments:
    ...
    local_dataset_loader: int = field(
        default=1, metadata={"help": "Whether to use the preprocessed dataset or not."}
    )
    ...
    exponential_sampling: Optional[bool] = field(
        default=True,
        metadata={
            "help": "Whether to use exponential sampling or follow the data distribution."
        },
    )
...
def main():
    ...
    if model_args.freeze_model_encoder:
        logger.info(f'Model encoder will remain frozen for initial '
                    f'{int(training_args.warmup_ratio * training_args.max_steps * 0.5)} steps!')

    if data_args.dataset_config_name == 'all':
        corpora = list(compute_sampling_rates()[0].keys())

    # Check if sampling is considered
    if data_args.exponential_sampling:
        _, _, train_probabilities = compute_sampling_rates()
        train_probabilities = list(train_probabilities.values())
        eval_probabilities = train_probabilities
        logger.info("Use exponential sampling!")
    else:
        _, train_probabilities, eval_probabilities = compute_sampling_rates()
        train_probabilities = list(train_probabilities.values())
        eval_probabilities = list(eval_probabilities.values())
        logger.info("Do not use exponential sampling!")

    # Downloading and loading datasets.
    logger.info(f"Use {data_args.dataset_name} dataset!")

    raw_datasets = {'train': [], 'test': []}
    if data_args.dataset_config_name == 'all':
        for subset_name in corpora:
            for subset in raw_datasets:
                raw_datasets[subset].append(load_dataset(data_args.dataset_name,
                                                         f'{subset_name}',
                                                         split=subset,
        ...
        for subset in raw_datasets:
            raw_datasets[subset].append(load_dataset(training_args.dataset_name,
    ...
    if len(raw_datasets) == 1:
        ...
        raw_datasets = {subset: interleave_datasets(raw_datasets[subset],
                                                    probabilities=train_probabilities if subset == 'train'
                                                    else eval_probabilities,
    ...
    if config.model_type == 'longformer':
        logger.info("Continued training from Longformer checkpoint")
    ...
    elif config.model_type == 'roberta' and \
    ...
    if data_args.line_by_line:
        ...
        def tokenize_function(examples):
            ...
            examples[text_column_name] = [
                line[:int(max_seq_length*6)] for line in examples[text_column_name] if len(line.split()) > 64 and not line.isspace()
            ]
            return tokenizer(
                examples[text_column_name],
                padding=padding,
        ...
        def tokenize_function(examples):
            ...
            for line in examples[text_column_name]:
                if len(line.split()) > 64 and not line.isspace():
                    ws_tokens = line.split(' ')
                    prev_idx = 0
                    for idx in range(max_sw_seq_length, len(ws_tokens) + max_sw_seq_length,  max_sw_seq_length):
                        if len(' '.join(ws_tokens[prev_idx:idx]).split()) > 64 and not ' '.join(ws_tokens[prev_idx:idx]).isspace():
